...
test_that("TWT works", {
  ...
  result<-ThreeWayTest::TWT(z_mat=z_mat, est_genetic_cor=diag(nrow = 5, ncol= 5), est_pheno_cor=diag(nrow = 5, ncol = 5),cutoff_value=c(0.2,0.4,0.6,0.8,1), coefficient_matrix=coefficient_matrix)
  expect_lte(result$p_value_final, 1)
  expect_gte(result$p_value_final, 0)
...
test_that("metaCCA works", {
  ...
  result<-ThreeWayTest::MGAS(z_vector=z_vector, est_genetic_cor = genotype_covariance, est_pheno_cor =phenotype_covariance)
  expect_lte(result, 1)
  expect_gte(result, 0)
})
